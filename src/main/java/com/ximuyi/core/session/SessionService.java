...
public class SessionService extends AkkaActor {
    ...
    private static long SECOND_TICK = Configs.getInstance().getInteger(ConfigKey.AKKA_SESSION_TIMEOUT, 60) * 1000L;
    ...
    private void onCommandUpMessage(MsChannelCommand channelCommand){
        ...
        try {
            ...
            long userId = ContextResolver.getUserHelper().getUserId(request);
            UserSession session = SessionManager.getInstance().getUserSession(userId);
    ...
    private void onCheckChannelUnLogin(ChannelCheck message){
        ...
        for (Map.Entry<Long, ChannelUnLogin> entry : channelUnLoginMap.entrySet()) {
            ...
            NetChannel decorator = entry.getValue().netChannel;
            ChannelUtil.channelClose(decorator, ChannelCloseReason.LOGIN_TIME_OUT, true);
